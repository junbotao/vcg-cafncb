function results = Copy_of_compare_all()
    % ================================================================
    ...
    cfg.macro_max_ls = 20;% Sets the maximum backtracking depth for the Armijo line search to ensure convergence.
    cfg.macro_lr = 0.05;% Default learning rate for SGD mode; note: inactive when macro_method is set to "bfgs".



    % VCG Curvature Governance
    cfg.vcg_lambda          = 0.1;      % Braking strength (lower = gentler braking, allows more optimization)
    ...
    cfg.density_bandwidth   = 1.2;       % KDE bandwidth (0.8 is a balanced smoothing value)
    ...
    cfg.eps0 = 1e-12;% Global numerical regularization constant to prevent division-by-zero in curvature (kappa) signals.
    
    
     % ================================================================
    % Micro QEC & Structural Consistency Enforcement (SEC)
    % ================================================================

    cfg.alpha_node = 0.08;     
    % Micro update step size.
    % Controls the magnitude of local (node-level) corrections.
    % Lower values reduce oscillation and prevent micro-level perturbations
    % from destabilizing macro convergence.

    cfg.proj_lambda = 1.;     
    % Structural consistency enforcement strength (SEC layer).
    % Higher values impose stronger projection toward linear/global
    % structural invariants, improving stability in higher dimensions.

    cfg.delta0 = 2;            
    % Micro search radius (lattice distance).
    % Defines the spatial neighborhood used in local micro-correction.
    % Larger values increase structural coupling but raise computation cost.

    cfg.delta2 = 1;            
    % Micro temporal audit window.
    % Determines the number of past steps used for micro-level consistency
    % checking. Smaller values preserve locality and causal structure.
    
    cfg.vcg_m = 1;  
% Curvature response order.
% Controls the exponent applied to the curvature signal (kappa)
% in the governance scaling term.
% Higher values increase sensitivity to large curvature deviations,
% making the braking mechanism more nonlinear.

cfg.rollback_gamma = 0.20;  
% Willow rollback mixing factor.
% Determines how strongly the state is reverted when a Willow
% trigger occurs.
% Higher values enforce a more aggressive rollback toward
